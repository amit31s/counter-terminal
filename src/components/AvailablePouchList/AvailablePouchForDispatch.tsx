...
interface AvailablePouchListProps {
  availablePouches: PouchDataForDespatch[];
  parentCallback: (pouch: PouchDataForDespatch) => void;
  pouchValueAddition?: string;
}

export const AvailablePouchForDispatch = ({
  availablePouches,
  parentCallback,
}: AvailablePouchListProps) => {
  ...
  const handleClick = (item: PouchDataForDespatch) => {
    if (!item.pouchID) {
      return;
    }
  ...
  const renderItem = ({ item, index }: { item: PouchDataForDespatch; index: number }) => {
    return (
      <TouchableHighlight
        {...touchProps}
